class BestHandResolver {
    companion object {
        ...
        fun resolve(cards: Array<out Card>): Hand {
            if (cards.size != 7) {
        ...
        private fun resolveStraightFlush(cards: Array<out Card>): Hand.StraightFlush? {
        ...
        private fun resolveQuads(cards: Array<out Card>): Hand.Quads? {
        ...
        private fun resolveFullHouse(cards: Array<out Card>): Hand.FullHouse? {
        ...
        private fun resolveFlush(cards: Array<out Card>): Hand.Flush? {
        ...
        private fun resolveStraight(cards: Array<out Card>): Hand.Straight? {
        ...
        private fun resolveTrips(cards: Array<out Card>): Hand.Trips? {
        ...
        private fun resolveTwoPair(cards: Array<out Card>): Hand.TwoPair? {
        ...
        private fun resolveOnePair(cards: Array<out Card>): Hand.OnePair? {
            val originalCards = cards.toMutableList()
            val group = cards.groupBy(Card::rank)
            val pairs = group.filter { it.value.size == 2 }
        ...
        private fun resolveHighCard(cards: Array<out Card>): Hand.HighCard {
            val kickers = cards.sliceArray(0..5)
            return Hand.HighCard(kickers, Kickers(*kickers))
        }

        private fun isFlush(cards: Array<out Card>): Boolean {
            if (cards.size != 5) {
                throw IllegalStateException("Wrong number of cards")
            }
